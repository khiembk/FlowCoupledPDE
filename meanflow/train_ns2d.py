"""
Training script for NavierStokes2D 3-process coupled flow.

Uses CoupledFlowNProc with unet_lite architecture (~21M trainable params for 3 nets).
Channels: u (vorticity/scalar), vx, vy.

Usage (from meanflow/):
    python train_ns2d.py \\
        --data_path /scratch/.../ns2d.pt \\
        --output_dir /scratch/.../ckpt/ns2d_gp \\
...
_MEANFLOW = Path(__file__).resolve().parent
sys.path.insert(0, str(_MEANFLOW))
...
N_PROC = 3  # NS2D: u, vx, vy
CHAN_NAMES = ["u", "vx", "vy"]


def get_args_parser():
    p = argparse.ArgumentParser("NS2D coupled flow training (CoupledFlowNProc)", add_help=True)
    # Data
    p.add_argument("--data_path",    required=True)
    p.add_argument("--train_ratio",  type=float, default=0.6667)
    p.add_argument("--val_ratio",    type=float, default=0.1667)
    # Training
...
def main(args):
    ...
    _, train_loader = build_ns2d_dataloader(split="train", shuffle=True,  pin_memory=args.pin_mem, **kw)
    ...
    # ── training loop ────────────────────────────────────────────────────────
    logger.info(f"Start training NS2D: epochs {args.start_epoch}–{args.epochs}")
