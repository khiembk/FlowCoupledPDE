...
class CoupledFlow(nn.Module):
    ...
    def __init__(self, arch, args, net1_configs, net2_configs, encoding_net_configs = "linear"):
        super().__init__()
        self.args = args

        self.net1 = arch(**net1_configs)
        self.net2 = arch(**net2_configs)
        self.encoding_net = arch(**encoding_net_configs)
        ...
        self.encoding_net_ema = init_ema(self.encoding_net, arch(**copy.deepcopy(encoding_net_configs)), args.ema_decay)
        ...
        for i, decay in enumerate(self.ema_decays):
            ...
            self.add_module(
                ...
                init_ema(self.encoding_net, arch(**copy.deepcopy(encoding_net_configs)), decay)
            )
    ...
    def update_ema(self):
        ...
        update_ema_net(self.encoding_net, self.encoding_net_ema, num_updates)
        ...
        for i in range(len(self.ema_decays)):
            ...
            update_ema_net(self.encoding_net, self._modules[f"encoding_net_ema{i+1}"], num_updates)
    ...
    def bezier_build_z_t_vt(self, source_1, source_2, target_1, target_2, t):
        ...
        zt_1 = (1.0 - t) * target_1 + t * source_1 + t*(1.0 -t)*self._encoding_net(1, source_1, source_2, target_1, target_2, t)
        zt_2 = (1.0 - t) * target_2 + t * source_2 + t*(1.0 -t)*self._encoding_net(1, source_1, source_2, target_1, target_2, t)
        # help me implemnt the velocity here 
    

    def _encoding_net(self, which, source_1, source_2, target_1, target_2, t, net=None):
        net = self.encoding_net if net is None else net
        return net(source_1, source_2, target_1, target_2, t)[which]
    ...
    def forward_local_loss(self, source_1, source_2, target_1, target_2):
        ...
        z_t_1, z_t_2, v_t_1, v_t_2 = self.bezier_build_z_t_vt(source_1, source_2, target_1, target_2, t_b)
