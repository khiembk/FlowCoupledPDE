"""
Save MeanFlow-GP rollout predictions for LV qualitative figures.

Run with pdemeanflow conda env.

Usage:
    python rollout/save_preds_lv_gp.py \
...
def _load_lv_model(ckpt_path, device):
...
@torch.no_grad()
def _rollout_lv(model, z0, n_steps, net1, net2):
    # z0: [B, 2, 256]
    src1, src2 = z0[:, 0:1], z0[:, 1:2]
    preds = []
    for _ in range(n_steps):
        p1, p2 = model.sample(src1, src2, net1=net1, net2=net2)
        preds.append(torch.cat([p1, p2], dim=1))  # [B, 2, 256]
        src1, src2 = p1, p2
    return preds
...
def main():
    ...
    # gt_all: [N_test, 2, n_t+1, 256]
    idxs = args.sample_idxs
    gt_sel = gt_all[idxs]   # [N_sel, 2, n_t+1, 256]
    torch.save(gt_sel, str(out_dir / "gt.pt"))
    print(f"GT saved: {gt_sel.shape}")

    ckpt_path = Path(args.ckpt_dir) / "lv512_gp" / args.ckpt_name
    print(f"Loading lv512_gp from {ckpt_path}")
    model = _load_lv_model(str(ckpt_path), device)

    z0_sel = gt_all[idxs, :, 0, :].to(device)   # [N_sel, 2, 256]
    preds = _rollout_lv(model, z0_sel, args.n_steps, model.net1, model.net2)
